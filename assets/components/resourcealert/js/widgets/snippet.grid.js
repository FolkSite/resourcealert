...
resourcealert.grid.Snippets = function (config) {
    config = config || {};
    ...
    Ext.applyIf(config, {
        ...
        , tbar: [{
            ...
            ,{
                xtype: 'button'
                , id: 'resourcealert-snippets-reload'
                , text: 'reload'
                , listeners: {
    ...
    resourcealert.grid.Snippets.superclass.constructor.call(this, config)
};
Ext.extend(resourcealert.grid.Snippets, MODx.grid.Grid, {
    ...
    , changeCategory: function (btn, e) {
        ...
        if (!this.changeCategoryWindow) {
            this.changeCategoryWindow = MODx.load({
                  xtype: 'resourcealert-snippet-window-change-category'
                  , record: r
                  , listeners: {
                    'success': {fn: function (r) {
                        // this.refresh();
                        var sels = this.getSelectionModel().getSelections();
                        var cat = Ext.getCmp('resourcealert-snippet-category-combo').lastSelectionText;
                        var s = this.getStore();
...
Ext.reg('resourcealert-grid-snippet', resourcealert.grid.Snippets);
...
resourcealert.window.ChangeCategory = function (config) {
    ...
    Ext.applyIf(config, {
        ...
        ,fields: [{
            ...
            ,id: 'resourcealert-snippet-category-combo'
...
Ext.reg('resourcealert-snippet-window-change-category', resourcealert.window.ChangeCategory);
